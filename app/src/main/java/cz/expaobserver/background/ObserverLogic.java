...
/**
 * @author Eugen on 7. 9. 2015.
 */
public class ObserverLogic implements SensorEventListener, LocationListener {
    ...
    private @ObserverLogic.TrailMeasureState int mState;
    ...
    private Callbacks mCallbacks = Callbacks.DUMMY;

    public static ObserverLogic getInstance(ObserverApplication application) {
        return new ObserverLogic(application);
    }

    private ObserverLogic(ObserverApplication application) {
        ...
        mRecordStore = application.getRecordStore();
...
//
        ...
        void onStateChanged(@TrailMeasureState int state);

        void onTimeChanged(long time);

        void onOrientationChanged(Vector3 vector);
    }
}
